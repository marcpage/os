...
#define GAIMessageThrow(condition) net::GetAddressInfoException::_throw(condition, #condition, __FILE__, __LINE__)

	class GetAddressInfoException : public msg::Exception {
		public:
			GetAddressInfoException(int value, const char *name, const char *file=NULL, int line=0) throw();
			GetAddressInfoException(const std::string &message, int value, const char *name, const char *file, int line) throw();
...
GAIException(EAI_FAIL);		GAIException(EAI_FAMILY);	GAIException(EAI_MEMORY);
GAIException(EAI_NONAME);	GAIException(EAI_OVERFLOW);	GAIException(EAI_PROTOCOL);
GAIException(EAI_SERVICE);	GAIException(EAI_SOCKTYPE);	GAIException(EAI_SYSTEM);
...
#undef GAIException
	...
	inline GetAddressInfoException::GetAddressInfoException(int value, const char *name, const char *file, int line) throw()
		:msg::Exception(_init("", name, value), file, line), _value(value) {}
	inline GetAddressInfoException::GetAddressInfoException(const std::string &message, int value, const char *name, const char *file, int line) throw()
		:msg::Exception(_init(message, name, value), file, line), _value(value) {}
	inline GetAddressInfoException::GetAddressInfoException(const GetAddressInfoException &other)
		:msg::Exception(other), _value(other._value) {}
	inline GetAddressInfoException &GetAddressInfoException::operator=(const GetAddressInfoException &other) {
	...
	inline void GetAddressInfoException::_throw(int value, const std::string &message, const char *file, int line) {
		if (0 != value) {
			switch(value) {
#define GAICaseClass(name) case name: throw name##_GAI(message, file, line)
				GAICaseClass(EAI_AGAIN);	GAICaseClass(EAI_BADFLAGS);	GAICaseClass(EAI_BADHINTS);
				GAICaseClass(EAI_FAIL);		GAICaseClass(EAI_FAMILY);	GAICaseClass(EAI_MEMORY);
				GAICaseClass(EAI_NONAME);	GAICaseClass(EAI_OVERFLOW);	GAICaseClass(EAI_PROTOCOL);
				GAICaseClass(EAI_SERVICE);	GAICaseClass(EAI_SOCKTYPE);	GAICaseClass(EAI_SYSTEM);
#undef GAICaseClass
				default:
	...
	inline std::string Address::name(Qualified qualified, Format format, Required required) const {
		...
		GAIMessageThrow( ::getnameinfo(get(), size(), const_cast<char*>(dnsName.data()), dnsName.size(), NULL, 0, flags) );
		return dnsName;
	}
}
