...
namespace crypto {
	...
	inline void SpecificSymmetricKey<SpecificCryptor>::encryptInPlace(const char *data, const size_t dataSize, const std::string &iv, char *encrypted, size_t &encryptedSize) const {
		__crypto_EncryptAssert(IVWrongSize, (iv.length() == SpecificCryptor::IVLength) || (iv.length() == 0));
	...
	struct OpenSSLAES {
		...
		*/
		static size_t _crypt(int enc, const void *key, const void *data, size_t length, void *out, size_t /*outBufferSize*/, const void *iv) {
			...
			__crypto_OSSLHandle(EVP_CipherInit_ex(context, cipher_type, NULL, reinterpret_cast<const unsigned char*>(key), reinterpret_cast<const unsigned char*>(iv), enc));
			...
			__crypto_OSSLHandle(EVP_CipherUpdate(context, outBuffer, &bytesWritten, reinterpret_cast<const unsigned char*>(data), length));
	...
	struct CommonCryptoKey {
		...
		static size_t _crypt(CCOperation op, const void *key, const void *data, size_t length, void *out, size_t outBufferSize, const void *iv) {
			...
			__crypto_CCHandle(CCCrypt(op, algorithm, options, key, keyLength, iv, data, length, out, outBufferSize, &outDataSize));
			return outDataSize;
		}
	};
